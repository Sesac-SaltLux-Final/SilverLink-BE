...
import com.aicc.silverlink.domain.session.repository.AuditLogRepository;
...
public class SessionController {
    ...
    public ResponseEntity<Page<AuditLog>> getAuditLogsByType(
        ...
        return ResponseEntity.ok(auditLogRepository.findByEventTypeOrderByCreatedAtDesc(eventType, pageable));
    }
    ...
    public ResponseEntity<Map<String, Long>> getSecurityStats() {
        ...
        return ResponseEntity.ok(Map.of(
                "duplicateLogins",
                auditLogRepository.countByEventTypeAndCreatedAtAfter("DUPLICATE_LOGIN_DETECTED", last24h),
                "forceLogins", auditLogRepository.countByEventTypeAndCreatedAtAfter("FORCE_LOGIN_EXECUTED", last24h),
                "sessionCreated", auditLogRepository.countByEventTypeAndCreatedAtAfter("SESSION_CREATED", last24h),
                "sessionInvalidated",
                auditLogRepository.countByEventTypeAndCreatedAtAfter("SESSION_INVALIDATED", last24h)));
    }
}
