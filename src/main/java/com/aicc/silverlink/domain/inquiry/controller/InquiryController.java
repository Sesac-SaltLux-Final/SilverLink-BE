...
public class InquiryController {
    ...
    public ResponseEntity<List<InquiryResponse>> getInquiries(@AuthenticationPrincipal User user) {
        return ResponseEntity.ok(inquiryService.getInquiries(user));
    }

    @GetMapping("/{id}")
    @Operation(summary = "문의 상세 조회", description = "특정 문의의 상세 내용을 조회합니다.")
    public ResponseEntity<InquiryResponse> getInquiry(@PathVariable Long id, @AuthenticationPrincipal User user) {
